...
<? if (is_array($Slider['video']) && count($Slider['video']) > 0) { ?>
    <div class="main-top">
        <? foreach ($Slider['video'] as $video) { ?>
            <video class="main-top__video main-top__image_mobile" loop="" autoplay="" playsinline="" muted=""
                   src="<?= CFile::GetPath($video['PROPERTIES']['VIDEO']['VALUE']); ?>" poster=""></video>
        <? } ?>
    </div>
<? } ?>

<? if (is_array($Slider['top']) && count($Slider['top']) > 0) { ?>
...
<? if (is_array($Slider['mobile']) && count($Slider['mobile']) > 0) { ?>
...
<? if (is_array($Slider['center']) && count($Slider['center']) > 0) { ?>
        ...
        <? foreach ($Slider['center'] as $center) { ?>
            <div class="center_slider_item">
                <a href="<?= $center['PROPERTIES']['LINK']['VALUE'] ? $center['PROPERTIES']['LINK']['VALUE'] : "javascript:void(0);"; ?>">
                    <img src="<?= CFile::GetPath($center['PROPERTIES']['IMG']['VALUE']); ?>"
                         alt="<?= $center['NAME'] ?>">
                </a>
            </div>
        <? } ?>
...
<? if (is_array($Slider['bottom']) && count($Slider['bottom']) > 0) { ?>
        ...
        <? foreach ($Slider['bottom'] as $bottom) { ?>
            <div class="bottom_slider_item">
                <a href="<?= $bottom['PROPERTIES']['LINK']['VALUE'] ? $bottom['PROPERTIES']['LINK']['VALUE'] : "javascript:void(0);"; ?>">
                    <img src="<?= CFile::GetPath($bottom['PROPERTIES']['IMG']['VALUE']); ?>"
                         alt="<?= $bottom['NAME'] ?>">
                </a>
            </div>
        <? } ?>
